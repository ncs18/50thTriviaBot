...
for (var i = 0; i < masterArray.length; i++) {
    ...
    var answer = masterArray[i].split("?")[1].split(":")[0];
    var pointValue = masterArray[i].split(":")[1];
...
for (var i = 0; i < userMoneyArray.length; i++) {
    ...
    var moneyAdd = Number(userMoneyArray[i].split(":")[1]);
    if (userAdd != "") {
        map[userAdd] = moneyAdd;
    }
}
...
class Bot {
    ...
    static checkMessage(message) {
        ...
        if (messageText.toLowerCase() == answerArray[indexA]) {
            ...
            //------------------------------------------------------------------------//
            if (!isNaN(map[messageUserID])) {
                map[messageUserID] =
                    map[messageUserID] + Number(pointArray[indexA]);
            } else {
                map[messageUserID] = Number(pointArray[indexA]);
            }
            ...
            try {
                fs.unlinkSync("userMoney.txt");
                fs.writeFileSync("userMoney.txt", map_string);
            } catch (err) {
                console.error(err);
            }
        ...
        // SLEEP
        var seconds = process.hrtime()[0] + 10 // Change this number to how long to sleep
        while (process.hrtime()[0] < seconds) {}
